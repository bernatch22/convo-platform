# `tenants._template.projects.example.evals.dag`

The reasoning that used to live in the docstrings of `tenants/_template/projects/example/evals/dag.py`; the code keeps one line per symbol.

## module

The shapes live in `convo.testing` — was the irreversible tool run and was the
...
`convo/testing/` with the words left behind in this file.
