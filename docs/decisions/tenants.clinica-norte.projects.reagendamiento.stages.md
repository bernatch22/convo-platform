# `tenants.clinica-norte.projects.reagendamiento.stages`

The reasoning that used to live in the docstrings of `tenants/clinica-norte/projects/reagendamiento/stages/__init__.py`; the code keeps one line per symbol.
...
paragraphs are shared (`prompts/_partials/` partials) and the hour helpers are shared
...
what it takes from the `prompts/_partials/` partials is how the clinic speaks, how it says an hour
