# `tenants.tienda-sur.projects.pedidos.evals.dag`

The reasoning that used to live in the docstrings of `tenants/tienda-sur/projects/pedidos/evals/dag.py`; the code keeps one line per symbol.

## module

The shapes live in `convo.testing.metrics.dag`; what a shop owns is what the questions
...
two files are the same graph builders with different nouns, which is what
