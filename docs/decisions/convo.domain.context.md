...
One definition, built once per job by `convo.session.router.resolve`, carried as the
...
Subclasses in `tenants/<id>/tenant.py` override it — `convo/` never imports
...
(`convo.tools.messages`) in the project's own register and language.
`backchannels` overrides the murmurs a barge-in filter ignores
(`convo.session.barge_in.SPANISH_BACKCHANNELS`) — data, so `convo/` knows one language.
`stt_gate` overrides how much voiced audio a transcript must have behind it
to be believed (`convo.session.stt_gate.GateOptions`), for a tenant on a noisier line.

The fields named in `convo.state.overrides.OVERRIDABLE` are the ones a
supervisor may change from the console without a deploy: `convo.state.overrides`
replaces them on the way out of the router (`convo.state.store.PipelineOverride`).
...
`convo/providers`, and an eval can measure a second model on the same goldens
(`convo.testing.reports.report --model`) without editing one of them.
...
tool that cannot work is not offered, `convo.telephony.human`.
...
contract describes — see `convo.tools.guard.mask`.
