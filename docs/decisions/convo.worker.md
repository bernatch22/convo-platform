...
`convo console` talks to the agent from the laptop microphone
(`--text` for the keyboard); `convo worker dev` registers against a
...
`convo.session.recordings.root()`, keyed by session id, and its path is written into
...
every job, and `convo.session.recordings.aim` is the one line that stops the file
