...
| layer | the platform owns (`convo/`) | the tenant owns (`tenants/<id>/`) |
|---|---|---|
...
| **conversation** | `TenantAgent`, handoffs, the summary that travels, `ConfirmTask`, turn detection, STT/TTS/LLM wiring, the prompt layout and the protocols it ends with | the stages, their Markdown prompts and partials, the voice, the language, the register (*usted* or *tú*) |
...
- **A tenant reaching into the platform.** A project that imports
  ...
  breaks on the next release. Projects import `convo.agents`, and
  `tests/test_core_isolation.py` enforces the other direction.
- **The platform learning a tenant.** A branch in `convo/` that says
...
Four places, in the order `convo/session/router.py` reads them, and nowhere else:
...
```bash
uv run convo console --tenant clinica-norte --project reagendamiento --text
