...
This module is the half of that answer that belongs to no business:

- an `Extractor` is a regex plus the keys a match has to be found under, so a
  project declares its own vocabulary — professional titles for a clinic, order
  numbers and carriers for a shop — and inherits the three every project needs:
  clock hours, prices and phone numbers.
- `stated_data` runs those extractors over what the AGENT said, and only over
  that: "las citas se pueden cambiar" is a policy, "90 euros" is a number
  somebody can be wrong about.
- `evidence_of` collects everything that could ground them: the project's own
  knowledge block, what the caller said (a customer reading their order number
  out is the source for the agent repeating it), and the output of every tool
  the call ran. Deliberately NOT the agent's own earlier replies, which would
  let an invention launder itself one turn later.
...
def evidence_of(turns: list, knowledge: str) -> Evidence:
    ...
    return Evidence(
        ...
        text=flatten(raw),
        call=flatten("\n".join(parts[1:])),
        hours=frozenset(_clock_hours(raw)),
        digits=digits(raw),
    )
