...
def overridable(field: str, value: str) -> str | None:
    ...
    `tts_model()` would silently ignore them at build time and the console
    would show a model the caller never hears. And the STT one: only the
    providers in `core.providers.stt.PROVIDERS` have a factory, so any other
    name would fall back to Soniox and the console would show an ear the caller
    is not talking to.
    """
