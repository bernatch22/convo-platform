...
from core.state.store import Route, SQLiteStore, Store

USAGE = "usage: python -m convo routes list | add <fleet> <key> <tenant> <project> [voice|chat]"
HEADER = f"{'fleet':<8} {'key':<18} {'tenant/project':<36} channel"


def main(argv: list[str], store: Store | None = None) -> int:
    """`list` prints every route; `add` registers or replaces one."""
