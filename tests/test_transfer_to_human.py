...
4. A REFER the carrier refuses is an ANSWER: `ok=False` means the caller is
   ...
   it to keep helping.
5. A browser call and a chat are told the truth. There is no phone leg to move,
   nothing is asked of the SFU, the attempt is still written down as
   `no_phone_leg`, and the model gets a `ToolError` with what it CAN offer.
...
from tests.test_transfer import CALLER, ROOM, FakeAPI, FakeRoom, FakeSession, Person
...
# ── 5. a browser call and a chat are told the truth ──────────────────────────


async def test_a_browser_call_is_refused_honestly_and_the_sfu_is_never_asked(tc, livekit) -> None:
    in_a_browser(tc)
    ...
    assert livekit.sip.transfers == [], "nothing was asked of the carrier"
